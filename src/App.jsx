...
const UI_STORAGE_KEY = "cashflow-ui-v1";

const texts = {
  de: {
    appTitle: "Cashflow-App",
    appSubtitle: "Mobile first, modern, mit Rollen, Deutsch/Russisch und Theme-Umschaltung.",
    login: "Einloggen",
    logout: "Ausloggen",
    email: "E-Mail",
    password: "Passwort",
    ...
    basedOnRules: "Schätzung auf Basis der Regeln",
    role: "Rolle",
    owner: "Inhaber",
    viewer: "Nur Ansicht",
    unknown: "unbekannt",
    ...
    exportNotReady: "CSV/Transaktionen bauen wir als Nächstes wieder darauf auf.",
    monday: "Montag",
    tuesday: "Dienstag",
    wednesday: "Mittwoch",
    thursday: "Donnerstag",
  ...
  ru: {
    appTitle: "Cashflow-App",
    appSubtitle: "Сначала для телефона: современно, с ролями, русским языком и тёмной темой.",
    login: "Войти",
    logout: "Выйти",
    email: "Эл. почта",
    password: "Пароль",
    ...
    basedOnRules: "Оценка на основе правил",
    role: "Роль",
    owner: "Владелец",
    ...
    exportNotReady: "CSV/транзакции мы добавим следующим шагом поверх этой базы.",
    monday: "Понедельник",
    tuesday: "Вторник",
    wednesday: "Среда",
    thursday: "Четверг",
...
function expandRuleForMonth(rule, month) {
  ...
  if (rule.frequency === "monthly") {
    return [{ type: rule.type, amount, category: rule.category || "Sonstiges" }];
  }

  if (rule.frequency === "weekly") {
    const days = datesForWeeklyInMonth(month, rule.weekday ?? 1);
    return days.map(() => ({ type: rule.type, amount, category: rule.category || "Sonstiges" }));
  }
...
function ruleToForm(rule) {
  return {
    ...
    note: rule.note || "",
    customMonthsText: Array.isArray(rule.months) ? rule.months.join(", ") : "",
  };
}
...
export default function App() {
  ...
  async function handleDeleteRule() {
    ...
    await loadRules(householdId);
    const remaining = rules.filter((r) => r.id !== ruleForm.id);
  ...
  const summary = useMemo(() => summarizeRulesForMonth(rules, selectedMonth), [rules, selectedMonth]);
          ...
          <div style={styles.controlsCard}>
            <div style={{ display: "grid", gap: 12 }}>
              ...
              <div style={styles.row}>
                <button style={{ ...styles.button, ...styles.buttonAccent }} onClick={() => setTab("rules")}>
                  {tr(lang, "rules")}
                </button>
          ...
          <div style={{ display: "grid", gap: 14 }}>
            <div style={styles.metrics}>
              <div style={styles.metric}>
                ...
                <div style={styles.metricValue}>{formatCurrency(summary.income)}</div>
                <div style={styles.metricHint}>{tr(lang, "basedOnRules")}</div>
              </div>
              <div style={styles.metric}>
                <div style={styles.metricLabel}>{tr(lang, "monthlyExpense")}</div>
                <div style={styles.metricValue}>{formatCurrency(summary.expense)}</div>
                <div style={styles.metricHint}>{tr(lang, "basedOnRules")}</div>
              </div>
              <div style={styles.metric}>
                <div style={styles.metricLabel}>{tr(lang, "monthlyNet")}</div>
                <div style={styles.metricValue}>{formatCurrency(summary.net)}</div>
                <div style={styles.metricHint}>{formatMonth(selectedMonth, lang)}</div>
              </div>
              <div style={styles.metric}>
                <div style={styles.metricLabel}>{tr(lang, "totalRules")}</div>
                <div style={styles.metricValue}>{rules.length}</div>
                <div style={styles.metricHint}>{tr(lang, "monthlyOverview")}</div>
              </div>
            </div>
            ...
            <div style={styles.grid2}>
              <div style={styles.card}>
                ...
                <div style={{ marginTop: 14, color: palette.sub, fontSize: 14 }}>{tr(lang, "exportNotReady")}</div>
              </div>
                ...
                {summary.categories.length === 0 ? (
                  ...
                  <div style={{ display: "grid", gap: 12 }}>
                    {summary.categories.map((cat) => {
                      const max = Math.max(...summary.categories.map((x) => x.value), 1);
            ...
            {(selectedRuleId || (isOwner && selectedRuleId === "__new__")) ? (
              <div style={styles.card}>
                <h2 style={styles.cardTitle}>
                  {selectedRuleId === "__new__" ? tr(lang, "newRule") : tr(lang, "editRule")}
                </h2>
                <div style={styles.cardSubtitle}>
                  {isOwner ? tr(lang, "ownerHint") : tr(lang, "viewerHint")}
                </div>
